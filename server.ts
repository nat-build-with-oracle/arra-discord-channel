...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
      ...
      inputSchema: {
        ...
        properties: {
          ...
          mention_users: {
            type: 'array',
            items: { type: 'string' },
            description: 'User ids to @-tag (pinged). Prepended to the message; allowed_mentions is scoped to exactly these ids.',
          },
          mention_roles: {
            type: 'array',
            items: { type: 'string' },
            description: 'Role ids to @-tag (pinged). Same scoping as mention_users.',
          },
        },
    ...
    },
    {
      name: 'create_thread',
      description:
        'Create a Discord thread (Hermes-style auto_thread). Returns the new thread id — pass it as chat_id to reply/react inside the thread. Thread → parent gate: the parent channel must be allowlisted.',
      inputSchema: {
        type: 'object',
        properties: {
          chat_id: { type: 'string', description: 'Parent channel id (from an inbound <channel> block).' },
          name: { type: 'string', description: 'Thread title.' },
          message_id: { type: 'string', description: 'Optional: attach the thread to this message (thread hangs off it).' },
          message: { type: 'string', description: 'Optional: an opening message to post inside the new thread.' },
          auto_archive_minutes: {
            type: 'number',
            description: 'Auto-archive after inactivity: 60, 1440, 4320, or 10080. Default 1440 (1 day).',
          },
        },
  ...
  try {
    switch (req.params.name) {
      case 'reply': {
        ...
        const mentionUsers = (args.mention_users as string[] | undefined) ?? []
        const mentionRoles = (args.mention_roles as string[] | undefined) ?? []
        const hasMentions = mentionUsers.length > 0 || mentionRoles.length > 0
        ...
        // Hermes-style tagging: prepend @-tokens and scope allowed_mentions to exactly
        // these ids, so stray "@..." text in the body can never ping anyone unintended.
        const mentionPrefix = [
          ...mentionUsers.map(id => `<@${id}>`),
          ...mentionRoles.map(id => `<@&${id}>`),
        ].join(' ')
        const chunks = chunk(mentionPrefix ? `${mentionPrefix} ${text}` : text, limit, mode)
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            const sent = await ch.send({
              ...
              ...(shouldReplyTo
                ...
                : {}),
              ...(hasMentions ? { allowedMentions: { users: mentionUsers, roles: mentionRoles } } : {}),
            })
            noteSent(sent.id)
      ...
      case 'create_thread': {
        ...
        return {
          content: [{ type: 'text', text: `thread created (id: ${thread.id}) — reply with chat_id=${thread.id}` }],
        }
      }
      case 'fetch_messages': {
...
async function handleInbound(msg: Message): Promise<void> {
  ...
  // Ack reaction — "อ่านแล้ว" receipt on EVERY delivered message, observe included
  // (Nat, 2026-07-10): the room sees the bot saw it, without a reply cluttering chat.
  // Default 👀; access.ackReaction overrides; explicit "" disables entirely.
  const access = result.access
  const ack = access.ackReaction === '' ? '' : (access.ackReaction ?? '👀')
  if (ack) {
    void msg.react(ack).catch(() => {})
  }
